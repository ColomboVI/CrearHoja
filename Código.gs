function doGet() {
  ...
  var nombre_hoja='EVAL '+this_sheet.getPrefijoEdicion()+((data.id.trim()!='')?(' # '+ data.id.trim()):'');
   ...
   ***********************************/
  var eval_ss_fichero = DriveApp.getFileById(this_sheet.getPrograma().getIdArchivoEvaluaciones())
  ...
  *************************/
  ...
  eval_ss.getSheetByName('Referencias').getRange('PROGRAMA').setValue(data.programa);
  eval_ss.getSheetByName('Referencias').getRange('PAIS').setValue(data.pais);
  eval_ss.getSheetByName('Referencias').getRange('ID').setValue(id_del_curso);
  eval_ss.getSheetByName('Referencias').getRange('PROFESOR').setValue(emailProfesor);
  eval_ss.getSheetByName('Referencias').getRange('NOMBREPROFESOR').setValue(data.profesor);
   if(data.enviarEmail){
     Logger.log("Se va enviar email") 
    var message = '<h3>Estos son los detalles del curso</h3> \n' +
                    '<ul><li> Id Curso : ' +id_del_curso  + '</li>'+
                    '<li> Nombre de hoja : ' +nombre_hoja  + '</li>' +
                    '<li> Enlace al curso : ' + eval_ss.getUrl()+ '</li></ul>';
     ...
     var checkinTabla = SpreadsheetApp.openById('1W-bXlh6FlBHAR74Lx05HJkUp8Z-hjqDpbARLicP2lfs').getSheetByName('Cursos RAW');
     var now = new Date();
   checkinTabla.appendRow([ generarEdicion(data), data.pais,idFecha(data)+idQuarter(data), data.fecha_inicio, data.programa ,sigla.replace("-", "") ,nombre_hoja, id_del_curso , eval_ss.getUrl(),"","","", now,userEmail]);
  ...
  function generarCheckIn(data)
{
  ...
  var nombre_hoja=this_sheet.getPrefijoEdicion()+((data.id.trim()!='')?(' # '+data.id.trim()):'');
  var eval_ss_fichero = DriveApp.getFileById(this_sheet.getPrograma().getIdPlantillaExcelSeguimientoCheckIn())
                      .makeCopy(nombre_hoja, DriveApp.getFolderById(idCarpetaCheckIn)); 
  //copiar valores de Pais y Programa
 // var id_del_curso = generarId(data);
  var eval_ss=SpreadsheetApp.openById(eval_ss_fichero.getId());

  eval_ss.getSheetByName('Datos basicos').getRange('PROGRAMA').setValue(data.programa);
  eval_ss.getSheetByName('Datos basicos').getRange('PAIS').setValue(data.pais);
  eval_ss.getSheetByName('Datos basicos').getRange('Q').setValue(quarter);
  eval_ss.getSheetByName('Datos basicos').getRange('ID').setValue(id_del_curso);
  eval_ss.getSheetByName('Datos basicos').getRange('FECHA_INI').setValue(data.fecha_inicio);
   if(data.enviarEmail){
     Logger.log("Se va enviar email") 
     var message = '<h3>Estos son los detalles del curso</h3> \n' +
                    '<ul><li> Id Curso : ' +id_del_curso  + '</li>'+
                    '<li> Nombre de hoja : ' +nombre_hoja  + '</li>' +
                    '<li> Creado por : ' +userEmail  + '</li>' +
                     '<li> Enlace al curso : ' + eval_ss.getUrl()+ '</li></ul>';
     MailApp.sendEmail(userEmail, "Detalles del curso ID " + id_del_curso, '', {
               htmlBody: message
               ,name: 'Hojas curso'             
  ...
  //Escribir en la hoja de cursos RAW   
     var checkinTabla = SpreadsheetApp.openById('1W-bXlh6FlBHAR74Lx05HJkUp8Z-hjqDpbARLicP2lfs').getSheetByName('Check IN RAW');
     var now = new Date();
   checkinTabla.appendRow([generarEdicionCheckIn(data), data.pais,idFechaCheckIn(data)+idQuarterCheckIn(data) ,data.programa , id_del_curso , data.fecha_inicio, nombre_hoja, eval_ss.getUrl(),"","","","","",now,userEmail]);
  ...
  if(data.enviarEmail){
     ...
     var message = '<h3>Estos son los detalles del curso</h3> \n' +
                    '<ul><li> Id Curso : ' +id_del_curso  + '</li>'+
                    '<li> Nombre de hoja : ' +nombre_hoja  + '</li>'+
                    '<li> Enlace al curso : <a href=' + eval_ss.getUrl()+' target="_blank"></li></ul>';
