function generarId(data){ 
  ...
  var id_curso = idPais(data) + "-" + idPrograma(data)+idFecha(data)+idQuarter(data) + "-" +generarEdicion(data);
  Logger.log(id_curso)
  return id_curso  
  }
...
var aux = [];
var cursosEspana = SpreadsheetApp.openById('1W-bXlh6FlBHAR74Lx05HJkUp8Z-hjqDpbARLicP2lfs').getSheetByName('Cursos RAW');
var rangoValores = cursosEspana.getRange(1, 1, cursosEspana.getLastRow(),cursosEspana.getLastColumn()).getValues();
...
for (var i = 0; i < rangoValores.length; i++){
  ...
  Logger.log("generarID: La edición de tu curso es :::: " + edicion)
